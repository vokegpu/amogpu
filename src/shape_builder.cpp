...
void shape_builder::init() {
	...
	"if (u_bool_texture) {\n"
	"color = texture(u_sampler_texture_slot, varying_attrib_vertexes.xy);\n"
	"color *= u_vec_color;\n"
	"}\n"
	"frag_color = color;\n"
	"}\n";
...
void shape_builder::invoke(const uint16_t &format, const amogpu::vec4f &color, GLuint texture) {
...
void shape_builder::draw(float x, float y, float w, float h) {
	...
	this->concurrent_gpu_data.rect[2] = x + w;
	this->concurrent_gpu_data.rect[3] = y + h;

	amogpu::log("i am pretty " + std::to_string(shape_builder::vertex_buffer));
